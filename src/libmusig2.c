...
//    signer->R_list = malloc(sizeof (secp256k1_pubkey*)*V*NR_MSGS);
    unsigned char x[SCALAR_BYTES];
    mcs->commlist = malloc(sizeof (secp256k1_keypair)*4);

    int i,j,k;
    i=0;
    for (k = 0; k < NR_MSGS; k++){
        for (j = 0; j < V; j++) {
//            signer->r_list[i] = (unsigned char*)malloc(SCALAR_BYTES);
...
void musig2_key_agg_coef(musig2_context *mc, unsigned char *ser_xonly_X, unsigned char *a, unsigned char *L, int n) {
    ...
    secp256k1_tagged_sha256(mc->ctx, a, tag, sizeof (tag), concat, sizeof (concat));
}
...
void musig2_aggregate_pubkey(musig2_context *mc, secp256k1_pubkey *pk_list, int n ){
    ...
    for(i=0; i<n;i++) {
        ...
        secp256k1_xonly_pubkey_from_pubkey(mc->ctx, &temp_X, NULL, temp_X_LIST[i]);
    ...
    return_val += !secp256k1_ec_pubkey_combine(mc->ctx, &X_, (const secp256k1_pubkey *const *)temp_X_LIST, n);
    secp256k1_xonly_pubkey_from_pubkey(mc->ctx, &mc->x_agg_X, &mc->par_X, &X_);
}
...
int musig2_calc_b(musig2_context *mc, musig_param *param) {
    ...
    for (j=0; j<V; j++){
        return_val += !secp256k1_xonly_pubkey_from_pubkey(mc->ctx, &xonly_temp, NULL, &mc->agg_R_list[j]);
        secp256k1_xonly_pubkey_serialize(mc->ctx, temp_X, &xonly_temp);
    ...
    /* Compute b */
    return_val += !secp256k1_tagged_sha256(mc->ctx, param->b, tag, sizeof (tag), catXR, sizeof(catXR));
...
int musig2_calc_R(musig2_context *mc, musig_param *param) {
    ...
    for (j=0; j<V; j++){
        ...
        memcpy(&Rb_LIST[j]->data[0], mc->agg_R_list[j].data, PK_BYTES);
        if (j == 0 && par == 0) {
        }
        else if (j == 0 && par == 1) {
            loop_flag += !secp256k1_ec_pubkey_negate(mc->ctx, Rb_LIST[j]);
        }
        else {
            if (j == 1){
                /* If j = 1 => b_LIST[j] = b .*/
                memcpy(&param->b_LIST[j][0], param->b, SCALAR_BYTES);
                loop_flag += !secp256k1_ec_pubkey_tweak_mul(mc->ctx, Rb_LIST[j], param->b_LIST[j]);
            }
            else{
                memcpy(&param->b_LIST[j][0], param->b_LIST[j-1], SCALAR_BYTES);
                /* Compute b * b^(j-1) */
                loop_flag += !secp256k1_ec_seckey_tweak_mul(mc->ctx, param->b_LIST[j], param->b) ;
                loop_flag += !secp256k1_ec_seckey_negate(mc->ctx, param->b_LIST[j]);
                loop_flag += !secp256k1_ec_pubkey_tweak_mul(mc->ctx, Rb_LIST[j], param->b_LIST[j]);
            }
        }
    }
    ...
    combine_flag += !secp256k1_ec_pubkey_combine(mc->ctx, &R, (const secp256k1_pubkey *const *)Rb_LIST, V) ;
    combine_flag += !secp256k1_xonly_pubkey_from_pubkey(mc->ctx, &xonly_temp, &par, &R);
    combine_flag += !secp256k1_xonly_pubkey_serialize(mc->ctx, param->sx_R, &xonly_temp);

    if (loop_flag != 0 || combine_flag != 0) return -11;
    else                                     return par;
...
void musig2_calc_c(musig2_context *mc, musig_param *param){
    ...
    if (secp256k1_tagged_sha256(mc->ctx, param->c, tag , sizeof (tag), catXRM, SCALAR_BYTES * 3))
        return;
}
...
int musig2_set_parsig(musig2_context_sig *mcs, musig_param *param){
    ...
    unsigned char x[SCALAR_BYTES];
    secp256k1_keypair_sec(mcs->mc->ctx, x, &mcs->keypair);
    ...
    for(j = 0; j<V; j++){
        ...
        secp256k1_keypair_sec(mcs->mc->ctx, sr_list[j], mcs->commlist[index+j]);
    ...
    return_val += !secp256k1_ec_seckey_tweak_mul(mcs->mc->ctx, mcs->parsig, mcs->a);
