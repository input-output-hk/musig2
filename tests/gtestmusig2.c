...
static int sign_partial(musig2_context_sig *mcs_list, musig2_partial_signatures* mps){
    int i, err;
    for (i = 0; i < NR_SIGNERS; i++) {
...
#define less_signers (NR_SIGNERS - 1) // Multi signature will be generated with less signature than registered signers.
    ...
    // Aggregate partial signatures for `less_signers`.
    err = musig2_aggregate_partial_sig(ctx, &mca, mps, pk_list, signature, less_signers);
    ASSERT_EQ(err, 1);

    // Verify the aggregated signature with secp256k1_schnorrsig_verify
    // Verification should fail for not enough signatures.
...
#define nr_participants  (NR_SIGNERS + 1) // We define more signers.
    ...
    err = sign_partial(&mcs_list[1], mps);
    ASSERT_EQ(err, 1);
...
TEST (musig2, previous_state) {
    ...
    err = sign_partial(mcs_list, mps1);
    ASSERT_EQ(err, 1);
    ...
    for (i = 0; i < NR_SIGNERS; i++) {
        ...
        for (k = 0; k < NR_MESSAGES; k++) {
            for (j = 0; j < V; j++, l++) {
                ...
                secp256k1_ec_pubkey_serialize(ctx, &serialized_batch_list[ind * ser_size], &ser_size, &batch_list[ind], SECP256K1_EC_COMPRESSED );
            }
        }
    }
    for (i = 0; i < NR_SIGNERS; i++)
